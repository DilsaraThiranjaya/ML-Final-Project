...
for cell in nb['cells']:
    if cell['cell_type'] == 'code' and 'Advanced feature extraction' in "".join(cell['source']):
        source = cell['source']
        # Find where drop_cols is defined
        new_source = []
        for line in source:
            if 'drop_cols =' in line:
                # Add geographic extraction before dropping
                new_source.append("\n")
                new_source.append("# Extract geographic features (City and State)\n")
...
print("Geographic features extracted and preprocessor aligned.")
